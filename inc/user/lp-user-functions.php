...
function learn_press_rest_prepare_user_questions( array $question_ids = array(), array $args = array() ) : array {
	...
	if ( $question_ids ) {
		foreach ( $question_ids as $id ) {
			...
			*/
			$paragraph = get_field('paragraph', $id);
			if ($paragraph) {
				$questionData['paragraph']['title'] = $paragraph->post_title;
				$questionData['paragraph']['content'] = $paragraph->post_content;
			}
			/*
			* Customized
			* لجميع اقسام الاختبارات
			* صورة
			*/
			if (has_post_thumbnail( $id )) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( $id ), 'single-post-thumbnail' );

				$questionData['thumbnail'] = $image[0];
			}
...
<li class="form-field<?php echo esc_attr( $cf_class ); ?>">
    ...
    <?php
						case 'tel':
							?>
    <input name="_lp_custom_register_form[<?php echo $value; ?>]" type="<?php echo $custom_field['type']; ?>"
