...
<?php
		...
		public function admin_notice_require_addon_version() {
			?>
<div class="notice notice-error">
    <p><?php echo( '<strong>LearnPress version ' . LEARNPRESS_VERSION . ' require Addon</strong> version 4.0.0 or higher' ); ?>
    </p>
</div>
<?php
		}
...
add_action("wp_ajax_change_paragraph", 'change_paragraph');
add_action("wp_ajax_check_relational_array", 'check_relational_array');
// Check the questions_id array from request , if the question has paragraph 
function check_relational_array() {
	$paragraph_id = [];
	// recevie the questions id array from data
	$questions_id = json_decode(stripslashes($_POST['data']));
	// check if the question has paragraph , and push the result to (paragraph_id)
	for ($i=0; $i < count($questions_id); $i++) { 
		$paragrah = get_field('paragraph', $questions_id[$i]);
		array_push($paragraph_id, $paragrah->ID);
	}
   if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
      $result = json_encode($paragraph_id);
      echo $result;
   }
   else {
...
//    if ( !wp_verify_nonce( $_REQUEST['nonce'], "change_paragraph_nonce")) {
//       exit("Not verfied");
//    }   
  $paragrah = update_field( 'paragraph', get_post($_REQUEST["paragraph_id"]), $_REQUEST["question_id"] );
   if(!$paragrah) {
      $result['type'] = "error";
      $result['paragrah'] = $paragrah;
   }
   else {
      $result['type'] = "success";
      $result['paragrah'] = $paragrah;
